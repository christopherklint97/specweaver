...
func main() {
	...
	// Create a custom chi router instead of using the built-in router
	chiRouter := NewChiAdapter()

	// Add chi's middleware
	chiRouter.Use(middleware.Logger)
	chiRouter.Use(middleware.Recoverer)
	chiRouter.Use(middleware.RequestID)
	chiRouter.Use(middleware.RealIP)

	// IMPORTANT: Add the ChiURLParamMiddleware to make chi compatible with SpecWeaver
	chiRouter.Use(ChiURLParamMiddleware)

	// Configure the router with SpecWeaver's generated routes
	api.ConfigureRouter(chiRouter, server)

	// Start server
	port := ":8080"
	log.Printf("Starting Pet Store API server with Chi router on http://localhost%s", port)
